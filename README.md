...
**你发了话，它一定从你那句话看起。** 即使你刚翻上去看历史，打字发送的那一刻也会立刻回到列表底部并开始跟随——冻结会被解掉，滚动会被触发。「我发了话当然要看着回复」是唯一没有例外的情形。

往上翻阅时正在生成的回复**不会**把你顶走，也不会继续刷新：手指按在屏幕上、或者你已经离开底部时，页面会定住当前这份列表；等你回到底部，它再跟上。列表右下角有「↓ 回到最新」可以随时跳回来。

收起键盘时你正在读的位置**不会跑掉**。键盘收起会让可视区在几百毫秒里连续变高，动画结束那一刻底栏又冒出来再缩一次——而这一屏的消息列表是「底部对齐」的，视口一变它就重新贴底，于是内容会先被冲到最下面、再被拉回来。做法是**可视区每变一次就按锚点钉回去，一次一帧**，不等动画结束。在底部时不记也不钉：那时列表本来就该跟着键盘走，「发话要能看见回复」这条不受影响。
...
**书架的版式**：两列封面网格，封面本身就是卡片——书名和状态压在封面底部，只给底部一层渐变遮罩，上半部分留给画面。「继续写」是网格**上方**的一整行（不是网格里的一格），一屏只有一个续读入口。「新建小说」是网格里的第一格，跟封面同尺寸同圆角、虚线边。五种状态里只有「生成大纲中」是真正有活儿在跑的，只有它会在卡片底边画一条 2dp 的进度线。
...
- **LaTeX 嵌套超过 64 层会降级**，具体表现跟写法有关，两种都实测过：连写的 `\frac` 只剩下最外层那个分数是真的排版，更深的都摊成普通文字；成片没配对的 `{` 则整条渲染不出来，直接退回显示 `$...$` 原文。真实公式远远到不了这个深度，但模型写出没配对的长公式是常事——以前那会直接把进程打爆（实测 3000 个 `{` 就炸）。**上限之内的行为完全没变，渲染不了也一律退回原文，不会崩。**
