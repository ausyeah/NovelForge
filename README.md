...
- **书架**是唯一的书籍列表。点封面 = 继续写这本书（写作是这个 app 的主动作）；长按 = 写这本书 / 阅读 / 备份 / 重命名 / 删除。阅读是这本书自己的次要动作，不是另一个列表——以前有「全部项目」和「书架」两个门指向同一份书单，两者互不相通，从书架进了一本书就回不来，也永远进不去写作界面。
- **灵感**是聊天。从书内「更多」里打开时，会话按这本书分桶：A 书聊的人设不会被重发进 B 书的提问。
- **设置**是一个 hub，不是设置列表本身。模型连接、外观、壁纸在这一页；用量账本和备份与导出是它的二级页。

书内是三层：**创作设置 → 大纲（书的 hub）→ 正文**。大纲页是书的中心，生成控制、章节目录、写这一章、记忆、问问这本书都在这儿，但「全自动」的开关不在标题栏里——它跟「一键全自动生成全书」按钮放在一起，因为它和那个按钮是同一个意图。写正文和创作设置是单任务页，不显示底部栏，但**返回键一直在**。
